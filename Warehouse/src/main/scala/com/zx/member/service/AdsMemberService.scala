...
import com.zx.member.bean.QueryResult
import com.zx.member.dao.DwsMemberDao
...
object AdsMemberService {
  ...
  def queryDetailApi(sparkSession: SparkSession, dt: String) = {
    ...
    result.mapPartitions(iter=>{
      iter.map(item=>(item.appregurl+"_"+item.dn+"_"+item.dt,1))
    }).groupByKey(_._1)
      .mapValues(_._2)
      .reduceGroups(_+_)
      .map(item=>{
        val keys: Array[String] = item._1.split("_")
        val appregual: String = keys(0)
        val dn=keys(1)
        val dt=keys(2)
        (appregual,item._2,dt,dn)
      })
