...
import com.zx.member.bean.{DwsMember, DwsMember_Result, MemberZipper, MemberZipperResult}
import com.zx.member.dao.DwdMemberDao
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.SaveNamespaceRequestProto
import org.apache.spark.sql.{DataFrame, SaveMode, SparkSession}

object DwsMemberService {
  def importMemberUseApi(session: SparkSession, dt: String): Unit = {
    import session.implicits._ //隐式转换
    val dwdMember = DwdMemberDao.getDwdMember(session).where(s"dt='${dt}'") //主表用户表
    val dwdMemberRegtype = DwdMemberDao.getDwdMemberRegType(session)
    val dwdBaseAd = DwdMemberDao.getDwdBaseAd(session)
    val dwdBaseWebsite = DwdMemberDao.getDwdBaseWebSite(session)
    val dwdPcentermemPaymoney = DwdMemberDao.getDwdPcentermemPayMoney(session)
    val dwdVipLevel = DwdMemberDao.getDwdVipLevel(session)
  ...
  def importMember(session: SparkSession, dt: String) = {
    import session.implicits._
    session.sql(
      "select uid,first(ad_id),first(fullname),first(iconurl),first(lastlogin)," +
        ...
        //              s"(select * from dwd.dwd_member where dt='20190722') a left join dwd.dwd_member_regtype b on a.uid=b.uid " +
        //                "and a.dn=b.dn left join dwd.dwd_base_ad c on a.ad_id=c.adid and a.dn=c.dn left join " +
        //                " dwd.dwd_base_website d on b.websiteid=d.siteid and b.dn=d.dn left join dwd.dwd_pcentermempaymoney e" +
        //                s" on a.uid=e.uid and a.dn=e.dn left join dwd.dwd_vip_level f on e.vip_id=f.vip_id and e.dn=f.dn "+
        "group by uid,dn,dt").coalesce(1).write.mode(SaveMode.Append).insertInto("dws.dws_member")

    //查询当天增量数据
    val dayResult = session.sql(s"select a.uid,sum(cast(a.paymoney as decimal(10,4))) as paymoney,max(b.vip_level) as vip_level," +
      s"from_unixtime(unix_timestamp('$dt','yyyyMMdd'),'yyyy-MM-dd') as start_time,'9999-12-31' as end_time,first(a.dn) as dn " +
      s"from dwd.dwd_pcentermempaymoney a join " +
      s"dwd.dwd_vip_level b on a.vip_id=b.vip_id and a.dn=b.dn where a.dt='$dt' group by uid"
    ).as[MemberZipper]

    //查询历史拉链表数据
    val historyResult = session.sql("select *from dws.dws_member_zipper").as[MemberZipper]

    //两份数据聚合  对end_time进行重新修改
    dayResult.union(historyResult).groupByKey(item => item.uid + "_" + item.dn)
      .mapGroups {
        case (key, iters) =>
          ...
          val list: List[MemberZipper] = iters.toList.sortBy(_.start_time)
