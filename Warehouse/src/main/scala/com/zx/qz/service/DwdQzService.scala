package com.zx.qz.service

import com.alibaba.fastjson.JSONObject
import com.zx.qz.bean.{DwdQzPaperView, DwdQzPoint, DwdQzQuestion}
...
object DwdQzService {
  def etlQzPaper(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzPaper.log").filter(item => {
  ...
  def etlQzPoint(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzPoint.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzCenter(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzCenter.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
  ...
  def etlQzQuestion(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzQuestionType(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzQuestionType.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzMemberPaperQuestion(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzMemberPaperQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzPointQuestion(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzPointQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzSiteCourse(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzSiteCourse.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzCourse(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzCourse.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzCourseEdusubject(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzCourseEduSubject.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzWebsite(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzWebsite.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzMajor(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzMajor.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzBusiness(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzBusiness.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzPaperView(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzPaperView.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzCenterPaper(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzCenterPaper.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzChapterList(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/zx/ods/QzChapterList.log").filter(item=>{
      val nObject: JSONObject = ParseJsonData.getJsonData(item)
  ...
  def etlQzChapter(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._ //隐式转换
    sc.textFile("/user/zx/ods/QzChapter.log").filter(item => {
      val obj = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
